...
s7sdk.Util.require("s7sdk.image.Resolution");
s7sdk.Util.require("s7sdk.image.Tile");
if (!s7sdk.View) {
	s7sdk.VIEW_ALIGN_LEFT = 1;
	s7sdk.VIEW_ALIGN_CENTER = 0;
	s7sdk.VIEW_ALIGN_RIGHT = 2;
	s7sdk.View = function(e, j, t, a, h, s, n, u, x, o, d, l, f, c, k, m, g, q) {
		s7sdk.Logger
				.log(
						s7sdk.Logger.FINER,
						"s7sdk.View constructor (canvas based) - url: %0, image size: %1 x %2, view size: %3 x %4",
						e, j, t, a, h);
		...
		this.transitionTime = s7sdk.Util.checkDefault(x, 0.5) * 1000;
		...
		var y;
		do {
			y = new s7sdk.Resolution(r++, p, b, e, this.fmt, this);
			this.resolutions.push(y);
			p = Math.round(p / 2);
			b = Math.round(b / 2)
		} while (y.nTiles() > 1);
		s7sdk.View.maxLevel = r - 1;
		this.resolutions.reverse();
		this.calcRes();
		var v = new s7sdk.TileAddress(this.resolutions[0].w,
				this.resolutions[0].h, 0, 0, 0);
		var w = this.resolutions[0].getCreateTile(v, 0);
		if (w.image != null && w.image.addEventListener != null) {
			w.image.parentView = this;
			w.image.addEventListener(s7sdk.Event.TILE_LOADED, this.onTileLoad,
					true);
			w.image.addEventListener(s7sdk.Event.TILE_FAILED, this.onTileLoad,
					true)
		}
		this.canvas = null;
	...
	s7sdk.View.prototype.findParentElement = function() {
		return this.parentView.obj
	};
	s7sdk.View.prototype.getDisplayElement = function() {
		return this.canvas
	};
	s7sdk.View.prototype.attach = function(b) {
		this.parentView = b;
		this.canvas = b.displayElement;
		var a = this.findParentElement();
	...
	s7sdk.View.prototype.onTileLoad = function(b) {
		var a = s7sdk.Event.getTarget(b);
		a.removeEventListener(s7sdk.Event.TILE_LOADED, this.onTileLoad, true);
		a.removeEventListener(s7sdk.Event.TILE_FAILED, this.onTileLoad, true);
		if (a.parentView.viewParent != null
				&& a.parentView.viewParent.onReadyToDislpay) {
			a.parentView.viewParent.onReadyToDislpay()
		}
		a.parentView.invalidate(true);
		if (a.parentView.loadResetImage) {
			a.parentView.loadImage()
		}
		a.parentView = null
	};
	s7sdk.View.prototype.loadImage = function() {
		...
		s7sdk.Logger.log(s7sdk.Logger.INFO,
				"s7sdk.View.loadImage - requestUrl: %0", a);
	...
	s7sdk.View.prototype.onErrorImage = function(b) {
		...
		s7sdk.Logger.log(s7sdk.Logger.WARNING,
				"s7sdk.View.onErrorImage - Reset image failed to load")
	};
	s7sdk.View.prototype.onAbortImage = function(b) {
		...
		s7sdk.Logger
				.log(s7sdk.Logger.WARNING,
						"s7sdk.View.onAbortImage - Reset image did not finish the load")
	};
	s7sdk.View.prototype.lowerTile = function(h, e) {
		var b = null;
		var g = h + 1;
		var f;
		while ((b == null || !b.loaded) && g <= this.resolutions[0].level) {
			var a = this.resolutions[0].level - g;
			var d = this.resolutions[a];
			var c = Math.floor(e.x() / Math.pow(2, g - h))
					+ Math.floor(e.y() / Math.pow(2, g - h))
					* Math.ceil(d.w / s7sdk.Enum.TILE.SIZE);
			b = d.getTile(c);
			g++
		}
		return b
	};
	s7sdk.View.prototype.isLowerTileLoaded = function() {
		var a = this.resolutions[0].getTile(0);
		return ((a != null) && (a.loaded))
	};
	...
	s7sdk.View.prototype.pinchZoom = function(g, e, a, h) {
		s7sdk.Logger.log(s7sdk.Logger.FINEST,
				"s7sdk.View.pinchZoom - x: %0, y: %1, scale: %2", g, e, a);
	...
	s7sdk.View.prototype.zoomClick = function(b, g, d) {
		...
		g *= this.devicePixelRatio;
		s7sdk.Logger.log(s7sdk.Logger.FINEST,
				"s7sdk.View.zoomClick - x: %0, y: %1, ctrlDown: %2", b, g, d);
		var e;
		if (d) {
			e = this.zoomScale
		} else {
	...
	s7sdk.View.prototype.draw = function() {
		...
		var l = this.viewToImage.clone();
		l.invert();
		var b = l.transformPoint(new s7sdk.Point2D(0, 0));
		var a = l.transformPoint(new s7sdk.Point2D(1, 1));
		var e = new s7sdk.Rectangle(b.x, b.y, a.x - b.x, a.y - b.y);
		var g = new s7sdk.Rectangle(0, 0, this.w, this.h);
		e = e.intersection(g);
		var c = new s7sdk.Rectangle(Math.round(e.x), Math.round(e.y), Math
				.round(e.width), Math.round(e.height));
		if (e.containsRect(g) == false) {
			if (c.y > 0) {
				m.clearRect(0, 0, this.canvas.width, c.y + 1)
			}
			if (c.height + c.y < this.canvas.height) {
				m.clearRect(0, c.height + c.y - 1, this.canvas.width,
						this.canvas.height - c.height + c.y + 1)
			}
			if (c.x > 0) {
				m.clearRect(0, 0, c.x + 1, this.canvas.height)
			}
			if (c.width + c.x < this.canvas.width) {
				m.clearRect(c.x + c.width - 1, 0, this.canvas.width - c.x
						+ c.width + 1, this.canvas.height)
			}
		}
		if (!this.resetImageLoaded
			...
			if (!this.isLowerTileLoaded()) {
				return
			}
			var h = this.viewToRes();
			h.invert();
			this.cleanUpCanvasAndroid4();
			this.res.draw(e, h, this);
			s7sdk.Logger.log(s7sdk.Logger.FINEST,
					"s7sdk.View.draw - Rendering from tiles: %0", e.toString())
		} else {
			var f = this.viewToImage.clone();
			f.scale(this.resetImage.width, this.resetImage.height);
			var d = f.transformRect(e);
			var j = new s7sdk.Rectangle(0, 0, this.resetImage.width,
					this.resetImage.height);
			var k = d.intersection(j);
			var i = Math.abs(this.viewToImage.d - this.initScale) > s7sdk.View.epsilon2;
			s7sdk.Logger.log(s7sdk.Logger.FINEST,
					"s7sdk.View.draw - Rendering from image: %0 to: %1", k
							.toString(), e.toString());
			s7sdk.Logger
					.log(
							s7sdk.Logger.FINEST,
							"s7sdk.View - inPan: %0, inPinch: %1, resetImageLoaded: %2, inTransition: %3, tileRender: %4, resized: %5",
							this.inPan, this.inPinch, this.resetImageLoaded,
							this.inTransition, this.tileRender, this.resized);
			if (!isNaN(k.x) && !isNaN(k.y) && k.height > 0 && k.width > 0) {
				this.cleanUpCanvasAndroid4();
				if (this.transparent) {
					if (e.x == 0 && e.y == 0 && e.width == this.canvas.width
							&& e.height == this.canvas.height) {
						this.canvas.width = e.width
					} else {
						m.clearRect(e.x, e.y, e.width, e.height)
					}
				}
				if (i || this.resized) {
					m.drawImage(this.resetImage, k.x, k.y, k.width, k.height,
							e.x, e.y, e.width, e.height)
				} else {
					m.drawImage(this.resetImage, 0, 0, this.resetImage.width,
							this.resetImage.height, Math.round(e.x), Math
									.round(e.y), this.resetImage.width,
							this.resetImage.height)
				}
			}
		}
		if (this.textoverlay != null) {
			m.font = "30px Times New Roman";
			m.fillStyle = "Black";
			m.fillText(this.textoverlay, this.canvas.width / 2 - 50,
	...
	s7sdk.View.prototype.cleanUpCanvasAndroid4 = function() {
	...
	s7sdk.View.prototype.invalidate = function(b) {
		var d = this.delayTileLoad ? this.delayTileLoad : s7sdk.Util
				.checkDefault(b, false);
		if (d == false
				&& this.tileRender
				&& this.resized == false
	...
	s7sdk.View.prototype.imagePixelsToViewPoint = function(b) {
		var c = this.viewToImage.clone();
		c.scale(this.imageWidth, this.imageHeight);
		c.invert();
		var a = c.transformPoint(b);
		a.x /= this.devicePixelRatio;
		a.y /= this.devicePixelRatio;
		return a
	};
	...
	s7sdk.View.prototype.zoomIn = function() {
		this.zoomClick(this.w / (2 * this.devicePixelRatio), this.h
				/ (2 * this.devicePixelRatio), false)
	};
	s7sdk.View.prototype.zoomOut = function() {
		this.zoomClick(this.w / (2 * this.devicePixelRatio), this.h
				/ (2 * this.devicePixelRatio), true)
	};
	...
	s7sdk.View.prototype.parentToClientX = function(b, c) {
		var d = c.getBoundingClientRect();
		var a = b.clientX - d.left;
		return a
	};
	s7sdk.View.prototype.parentToClientY = function(b, c) {
		var d = c.getBoundingClientRect();
		var a = b.clientY - d.top;
		return a
	}
};