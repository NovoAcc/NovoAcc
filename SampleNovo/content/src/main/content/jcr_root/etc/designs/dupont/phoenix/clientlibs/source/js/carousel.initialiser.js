        ...
        api = (function () {
            ...
            function manageNavigationVisibility(carousel, next, prev, method) {
                if (elementIsFullyVisible(carousel, 'first', method)) {
                    $(prev, context).addClass('jcarousel-prev-disabled jcarousel-prev-disabled-horizontal');
                    $(prev, context).attr("disabled", true);
                } else {
                    $(prev, context).removeClass('jcarousel-prev-disabled jcarousel-prev-disabled-horizontal');
                    $(prev, context).attr("disabled", false);
                }

                if (elementIsFullyVisible(carousel, 'last', method)) {
                    $(next, context).addClass('jcarousel-next-disabled jcarousel-next-disabled-horizontal');
                    $(next, context).attr("disabled", true);
                } else {
                    $(next, context).removeClass('jcarousel-next-disabled jcarousel-next-disabled-horizontal');
            ...
            function carouselResize() {
                ...
                var width = $(options.carousel).width(),
                    ...
                    w = width / options.itemsToShow,
                    pctg = (w * 100) / width,
                    px = (width * (pctg / 100)) + 1,
                    outerWidth = $(options.items).first().outerWidth(true) - $(options.items).first().width();

                $(options.items).each(function (i, item) {
                    $(item).width(px - outerWidth);
                });
                setAttrs();
                logMsg(options);
