...
inlineEditing: 300
    ...
    /**
     * True if a click (during inplace editing) has already been handled by the currently
     * edited component; used to distinguish clicks on "other" components and on no
     * components, which both finish inplace editing, but are handled in different places
     * of the code ({@link CQ.wcm.EditRollover#handleClick} vs.
     * {@link CQ.WCM#clearSelection}).
     * @private
     * @type Boolean
     */
    isInplaceEditClickHandled: false,
    ...
    getParentFromDOM: function() {
         ...
         // find the editable for an element
		function getEditable(element) {
            ...
            for (var path in editables) {
                if (editables.hasOwnProperty(path)) {
                    ...
                    if (editable && editable.element && editable.element.dom) {
                        if (element === editable.element.dom) {
                            return editable;
                        }
                    }
                }
        ...
        if (this.parentEditable === undefined) {
            ...
            while (element = element.parentNode) {
                // look up if an editable exists for that ancestor element
                var editable = getEditable(element);
                if (editable) {
                    // hit: store result and return
                    this.parentEditable = editable;
                    break;
                }
            }
            if (!element) {
                // no parent editable, this editable is a "root"
                this.parentEditable = null;
            }
        }
...
//            }
    ...
    replaceElement: function(newElementHTML) {
    ...
    applyConfigDefaults: function(config, defaults) {
        ...
        if (this.getTargetParent()
            ...
            if (config.actions.indexOf(CQ.wcm.EditBase.COPYMOVE) !== -1) {
                config.actions.splice(config.actions.indexOf(CQ.wcm.EditBase.COPYMOVE), 1);
            }

            // we don't need the "New..." action unless it's a parsys
            if (config.actions.indexOf(CQ.wcm.EditBase.INSERT) !== -1
                    && this.getResourceType() !== CQ.wcm.EditBase.PARSYS_NEW) {
                config.actions.splice(config.actions.indexOf(CQ.wcm.EditBase.INSERT, 1));
            }

            //remove the trailing separator (for aestethic reasons)
            if (config.actions.lastIndexOf("-") == config.actions.length - 1) {
                config.actions.splice(config.actions.lastIndexOf("-"), 1);
            }
        }
    ...
    addTargetingActions: function(items, ctrlDef, isTargeted, config) {
        ...
        // Temporary Disable the target menu. Remove following line to re-enable target option.
    ...
    inplaceEditingOutOfAreaClick: function() {
        ...
        if (comp && !isLocked) {
            // a component is currently inplace edited: leave selection handling to the
            // corresponding select handler if the click was on a component
            var isClickHandled = comp.isInplaceEditClickHandled;
            if (isClickHandled) {
                comp.isInplaceEditClickHandled = false;
                return;
            }
     ...
     * @private
     */
    handleInplaceEditingInitError: function(errObj, loadMask) {
        if (loadMask) {
            loadMask.hide();
            loadMask.rootDiv.parentNode.removeChild(loadMask.rootDiv);
        }
