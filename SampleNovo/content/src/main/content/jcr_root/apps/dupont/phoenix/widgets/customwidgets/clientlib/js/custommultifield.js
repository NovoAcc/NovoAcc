		...
		.extend(
				CQ.form.CompositeField,
				{
					...
					constructor : function(config) {
									...
									.push({
										...
										items : [
												"->",
												{
													...
													handler : function() {
														if (config.fieldConfig.limit == 0
																|| list.items
																		.getCount() <= config.fieldConfig.limit) {
															list.addItem();
														} else {
															CQ.Ext.Msg
																	.show({
																		title : 'Limit reached',
																		msg : 'You are only allowed to add '
																				+ config.fieldConfig.limit
						...
						CQ.form.MultiField.superclass.constructor.call(this,
								config);
										...
										function() {
											...
											if (this.fieldWidth > 0) {
												for ( var i = 0; i < this.items.length; i++) {
													try {
														this.items.get(i).field
																.setWidth(this.fieldWidth);
													} catch (e) {
														CQ.Log
																.debug("CQ.form.MultiField#initComponent: "
																		+ e.message);
													}
												}
											}
					...
					calculateFieldWidth : function(item) {
						try {
							this.fieldWidth = this.getSize().width - 2
									* this.bodyPadding; // total row width
							for ( var i = 0; i < item.items.length; i++) {
								var button = item.items.get(i);
								// subtract each button
								if (button.items && button.items.length > 0
										&& button.items.get(0) === item.field) {
					...
					addItem : function(value) {
						...
						try {
							...
							CQ.Log.debug("CQ.form.MultiField#addItem: "
									+ e.message);
						}
					},
					...
					setValue : function(value) {
						...
						oldItems.each(function(item/*, index, length*/) {
							if (item instanceof CQ.form.MultiField.Item) {
								...
								this.findParentByType("form").getForm().remove(
										item);
							}
						}, this);
						this.doLayout();
...
CQ.Ext.reg("custommultifield", CQ.form.MultiField);
 ...
 * @param {Object} config The config object
 */
CQ.form.MultiField.Item = CQ.Ext
		.extend(
				CQ.Ext.Panel,
				{

					constructor : function(config) {
						var fieldConfig = CQ.Util
								.copyObject(config.fieldConfig);
						...
						CQ.form.MultiField.Item.superclass.constructor.call(
								this, config);
							...
							if (fieldConfig.orderable) {
								...
								move
										.on(
												"render",
												function() {
													move.dragZone = new CQ.Ext.dd.DragZone(
															move.getEl(),
															{
																item : item,
																...
																getDragData : function(
																		e) {
																	var sourceEl = move.ownerCt
																			.getEl();
																	sourceEl
																			.setVisibilityMode(CQ.Ext.Element.DISPLAY);
																	if (sourceEl) {
																		d = sourceEl.dom
																				.cloneNode(true);
																		d.id = CQ.Ext
																...
																onInvalidDrop : function(
																		target,
																		e, id) {
																	CQ.Ext.dd.DragZone.superclass.onInvalidDrop
																			.call(
																					this,
																					target,
																					e,
					...
					createDropTarget : function() {
						...
						this.dropTarget = new CQ.Ext.dd.DropTarget(
								this.getEl(),
								{
									item : item,
									...
									showTargetLine : function(el, position) {
										...
										CQ.form.MultiField.getTargetLine()
					...
					move : function(toItem, position) {
						...
						index = index > parent.items.getCount() ? parent.items
								.getCount() : index;
...
CQ.form.MultiField.getTargetLine = function() {
	if (!CQ.form.MultiField.TargetLine) {
		CQ.form.MultiField.TargetLine = CQ.Util
				.build(
						{
							"xtype" : "box",
							"renderTo" : "CQ",
		...
		CQ.form.MultiField.TargetLine.getEl().setVisibilityMode(
				CQ.Ext.Element.DISPLAY);
	}
